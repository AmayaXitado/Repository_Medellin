...
@php
    ...
    $boton = 'flex flex-col items-center justify-center gap-1.5 rounded-md border border-slate-300 px-3 py-4
              text-sm font-medium text-slate-700 hover:bg-slate-50 active:bg-slate-100
              dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-700 dark:active:bg-slate-600';
@endphp

@section('contenido')

<h1 class="text-lg font-semibold text-slate-900 dark:text-slate-100">Enviar un documento</h1>

@if($proposito)
    <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">{{ $proposito }}</p>
@endif
...
<form method="POST" action="{{ route('envio.recibir', ['token' => $token]) }}"
    ...
    <div>
        <label for="archivo" class="block text-sm font-medium text-slate-700 dark:text-slate-300">
            Archivo *
        </label>
        ...
        <input id="archivo" name="archivo" type="file" required accept="{{ $formatos }}"
               class="mt-1 block w-full text-sm text-slate-600 file:mr-3 file:rounded-md file:border-0
                      file:bg-slate-200 file:px-3 file:py-1.5 file:text-sm file:font-medium
        ...
        <div id="selector-archivo" class="mt-2 hidden">
            <div class="grid grid-cols-2 gap-2">
                <button type="button" id="boton-camara" class="{{ $boton }}">
                    <svg class="size-6 text-slate-500 dark:text-slate-400" fill="none" stroke="currentColor"
            ...
            <div id="archivo-elegido" class="mt-3 hidden items-center gap-3 rounded-md bg-slate-50 p-2
                                             dark:bg-slate-900">
                <img id="archivo-miniatura" alt="" class="hidden size-12 shrink-0 rounded object-cover">
                <p id="archivo-nombre" class="min-w-0 flex-1 truncate text-sm text-slate-700 dark:text-slate-300"></p>
                <button type="button" id="boton-quitar"
                        class="shrink-0 rounded-md px-2 py-1 text-xs font-medium text-slate-500 hover:text-slate-900
    ...
    <div>
        <label for="mensaje" class="block text-sm font-medium text-slate-700 dark:text-slate-300">
            Mensaje <span class="font-normal text-slate-500 dark:text-slate-400">(opcional)</span>
        </label>
        <textarea id="mensaje" name="mensaje" rows="3" maxlength="1000"
                  ...
                  class="mt-1 w-full rounded-md border-slate-300 text-sm shadow-sm
                         dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">{{ old('mensaje') }}</textarea>
    </div>

    <button class="w-full rounded-md bg-sky-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-sky-700">
        Enviar documento
    </button>
</form>
